...
/**
 * Manages an Appium mobile device session and provides methods for interacting with mobile
 * browsers. This is the mobile counterpart of {@link Browser}, allowing Appium to evolve
...
public class MobileDevice {
   ...
   *     <p>precondition: xpath != null precondition: !xpath.isEmpty()
   */
   ...
   *     <p>precondition: class_name != null
   */
  ...
  private static int extractViewportWidth(WebDriver driver) {
    ...
    int viewportWidth =
        Integer.parseInt(js.executeScript(JS_GET_VIEWPORT_WIDTH, new Object[0]).toString());
  ...
  private static int extractViewportHeight(WebDriver driver) {
    JavascriptExecutor js = (JavascriptExecutor) driver;
    int result =
        Integer.parseInt(js.executeScript(JS_GET_VIEWPORT_HEIGHT, new Object[0]).toString());
    return result;
  }
}
