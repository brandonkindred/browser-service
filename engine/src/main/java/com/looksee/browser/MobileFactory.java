 ...
 *
 * <p>This class is the mobile counterpart of {@link BrowserFactory} and allows Appium to evolve
...
public final class MobileFactory {
  ...
  public static WebDriver createDriver(String platformType, URL serverUrl)
    ...
    switch (platformType.toLowerCase()) {
      case "android":
        return openWithAndroid(serverUrl);
      case "ios":
        return openWithIOS(serverUrl);
      default:
  ...
  /**
   * Creates a fully configured MobileDevice instance using BrowserStack. BrowserStack runs Appium
   ...
   * @return the created MobileDevice
   *     <p>precondition: platformType != null precondition: hubUrl != null precondition: properties
   *     != null
   */
  public static MobileDevice createBrowserStackMobileDevice(
      String platformType, URL hubUrl, BrowserStackProperties properties) {
    assert platformType != null;
    assert hubUrl != null;
  ...
  /**
   * Creates a RemoteWebDriver configured for BrowserStack mobile testing. Uses RemoteWebDriver (not
   * AndroidDriver/IOSDriver) since BrowserStack handles the Appium server-side; the client just
   * needs to send capabilities.
   */
  private static WebDriver createBrowserStackMobileDriver(
      String platformType, URL hubUrl, BrowserStackProperties properties) {
    DesiredCapabilities caps = new DesiredCapabilities();
    ...
    switch (platformType.toLowerCase()) {
      case "android":
        ...
        caps.setCapability(
            "device",
            properties.getDeviceName() != null ? properties.getDeviceName() : "Samsung Galaxy S23");
        break;
      case "ios":
        caps.setCapability("platformName", "iOS");
        caps.setCapability("browserName", "Safari");
        caps.setCapability(
            "device",
            properties.getDeviceName() != null ? properties.getDeviceName() : "iPhone 15");
        break;
      default:
        throw new WebDriverException("Unsupported mobile platform type: " + platformType);
    }
