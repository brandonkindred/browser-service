...
class ControllerHttpTest {
  ...
  void readyzReturns200WhenReady() throws Exception {
    when(readinessService.probe())
        .thenReturn(
            new ReadinessResponse(
                "ready", List.of(new HubStatus("http://x/wd/hub", true)), List.of()));
  ...
  void readyzReturns503WhenDegraded() throws Exception {
    when(readinessService.probe())
        .thenReturn(
            new ReadinessResponse(
                "degraded", List.of(new HubStatus("http://x/wd/hub", false)), List.of()));
  ...
  void screenshotBinaryResponse() throws Exception {
    ...
    mvc.perform(
            post("/v1/sessions/" + id + "/screenshot")
                ...
                .content(
                    json.writeValueAsString(
                        new ScreenshotRequest(ScreenshotStrategy.VIEWPORT, null))))
        .andExpect(status().isOk())
  ...
  void screenshotBase64Response() throws Exception {
    ...
    mvc.perform(
            post("/v1/sessions/" + id + "/screenshot")
                ...
                .content(
                    json.writeValueAsString(
                        new ScreenshotRequest(ScreenshotStrategy.VIEWPORT, PngEncoding.BASE64))))
        .andExpect(status().isOk())
  ...
  void domRemove204() throws Exception {
    ...
    mvc.perform(
            post("/v1/sessions/" + id + "/dom/remove")
                ...
                .content(
                    json.writeValueAsString(
                        new DomRemoveRequest(DomRemovePreset.DRIFT_CHAT, null))))
        .andExpect(status().isNoContent());
  }
  ...
  void mouseMove204() throws Exception {
    ...
    mvc.perform(
            post("/v1/sessions/" + id + "/mouse/move")
                ...
                .content(
                    json.writeValueAsString(
                        new MouseMoveRequest(MouseMoveMode.OUT_OF_FRAME, null, null))))
        .andExpect(status().isNoContent());
  }
