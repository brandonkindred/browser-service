...
class ReadinessServiceTest {
    ...
    void readyWhenSeleniumReachableAndAppiumEmpty() {
        ...
        assertThat(probe.seleniumHubs()).singleElement().satisfies(hub -> assertThat(hub.reachable()).isTrue());
    ...
    void degradedWhenSeleniumUnreachable() {
        ...
        assertThat(probe.seleniumHubs()).singleElement().satisfies(hub -> assertThat(hub.reachable()).isFalse());
    }
    ...
    void readyWhenEitherAppiumOrSeleniumReachableAndOtherEmpty() {
        ReadinessService svc = new ReadinessService(
                props("http://127.0.0.1:" + port, "http://127.0.0.1:" + port));
        assertThat(svc.probe().status()).isEqualTo("ready");
    }
    ...
    void degradedWhenAppiumConfiguredButAllUnreachable() {
        ReadinessService svc = new ReadinessService(
                props("http://127.0.0.1:" + port, "http://127.0.0.1:1"));
