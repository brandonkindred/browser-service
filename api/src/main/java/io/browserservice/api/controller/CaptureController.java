...
public class CaptureController {
    ...
    @PostMapping
    @Operation(summary = "Capture a URL end-to-end in a single call",
            operationId = "capture")
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "429", description = "Concurrent session cap exceeded",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
            @ApiResponse(responseCode = "502", description = "Upstream unavailable",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
    ...
    @Operation(summary = "Fetch a deferred capture screenshot",
            operationId = "getCaptureScreenshot")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "PNG bytes"),
            @ApiResponse(responseCode = "404", description = "Capture not found",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
            @ApiResponse(responseCode = "410", description = "Capture expired",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
