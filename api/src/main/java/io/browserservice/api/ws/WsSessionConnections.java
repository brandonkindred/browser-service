...
/**
 * Per-session set of WebSocket connections currently bound to that session, used so the WS event
 * watchers (alerts, console, navigation) can fan out a single tick to every connected listener. The
 * booleans returned by {@link #attach} and {@link #detach} let the {@code WatcherCoordinator} start
 * watchers exactly once on the first attach and stop them exactly once on the last detach.
 */
@Component
