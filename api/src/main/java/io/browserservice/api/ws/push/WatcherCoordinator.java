 ...
 *
 * <ul>
 *   <li>Watchers start when the <strong>first</strong> WS connection binds to a session.
 *   <li>Watchers stop when the <strong>last</strong> connection unbinds, or when a tick discovers
 *       the session has been removed from the registry (manual close, reaper).
 *   <li>All scheduling shares the single application-wide {@code ws-scheduler}; we never allocate
...
public class WatcherCoordinator {
  ...
  private final ConcurrentHashMap<UUID, List<ScheduledFuture<?>>> running =
      new ConcurrentHashMap<>();

  /**
   * Per-session lock serializing first-attach and last-detach against each other so a concurrent
   * {@code last-detach + first-attach} interleaving can't cancel newly started futures (or leave
   * orphans). Locks live for the lifetime of the JVM — bounded by the session-cap, so the leak is
   * finite and small.
   */
  private final ConcurrentHashMap<UUID, ReentrantLock> sessionLocks = new ConcurrentHashMap<>();

  public WatcherCoordinator(
      WsSessionConnections connections,
      ...
      for (SessionEventWatcher w : watchers) {
        ...
        ScheduledFuture<?> f =
            scheduler.scheduleAtFixedRate(
                () -> safeTick(sessionId, w), period, period, TimeUnit.MILLISECONDS);
        futures.add(f);
      }
  ...
  private List<SessionEventWatcher> buildWatchers(UUID sessionId, SessionHandle handle) {
    ...
    if (props.alertPushEnabled()) {
      out.add(
          new AlertWatcher(
              sessionId, handle, locks, broadcaster, props.alertPollMs(), lockTimeout));
    }
    if (props.consolePushEnabled()) {
      out.add(
          new BrowserLogDrain(
              sessionId, handle, locks, broadcaster, props.consolePollMs(), lockTimeout));
    }
    if (props.navigationPushEnabled()) {
      out.add(
          new NavigationWatcher(
              sessionId, handle, locks, broadcaster, props.navigationPollMs(), lockTimeout));
    }
    return out;
  }
  ...
  private void safeTick(UUID sessionId, SessionEventWatcher watcher) {
    ...
    try {
      ...
      log.debug(
          "watcher tick threw session={} watcher={}: {}",
          sessionId,
