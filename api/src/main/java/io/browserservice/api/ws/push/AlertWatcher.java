...
/**
 * Polls {@code driver.switchTo().alert()} on a short cadence and pushes {@code
 * event{kind:"alert.appeared"}} on the rising edge — not once per tick while the alert remains
 * visible, and not after dismissal.
 *
 * <p>Reuses {@link AlertService#peekAlert(SessionHandle)} (the lock-free helper extracted for this
 * watcher) so the alert detection stays consistent with the REST surface.
 */
public final class AlertWatcher implements SessionEventWatcher {
  ...
  public void tick() {
    locks
        .tryDoWithLock(handle, lockTimeoutMs, AlertService::peekAlert)
  ...
  private void emitOnRisingEdge(AlertStateResponse state) {
    ...
    if (state.present() && !wasPresent) {
      broadcaster.broadcast(
          sessionId,
          EventFrame.of(
              "alert.appeared", Map.of("text", state.text() == null ? "" : state.text())));
    }
  }
}
