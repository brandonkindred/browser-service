...
public class ReadinessService {
    ...
    public ReadinessResponse probe() {
        ...
        boolean seleniumReady = !seleniumHubs.isEmpty()
                && seleniumHubs.stream().anyMatch(HubStatus::reachable);
        boolean appiumConfigured = !appiumServers.isEmpty();
        boolean appiumReady = !appiumConfigured
                || appiumServers.stream().anyMatch(HubStatus::reachable);
    ...
    private List<HubStatus> probeAll(String commaSeparated, String statusPath) {
        ...
        for (String raw : Arrays.stream(commaSeparated.split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
                .toList()) {
