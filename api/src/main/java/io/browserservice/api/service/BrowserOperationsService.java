...
public class BrowserOperationsService {
    ...
    public byte[] pageScreenshot(UUID sessionId, io.browserservice.api.dto.ScreenshotStrategy strategy) {
        SessionHandle handle = registry.get(sessionId);
        return locks.doWithLock(handle, h -> {
            try {
    ...
    public void removeDom(UUID sessionId, DomRemoveRequest req) {
        ...
        if (req.preset() == DomRemovePreset.BY_CLASS && (req.value() == null || req.value().isBlank())) {
    ...
    private BufferedImage renderPageScreenshot(SessionHandle h, io.browserservice.api.dto.ScreenshotStrategy strategy)
            throws IOException {
        if (h.isMobile()) {
