...
public class CaptureService {
    ...
    public CaptureService(SessionRegistry registry, SessionLocks locks,
                          DriverFactory drivers, CaptureScreenshotCache cache,
    ...
    public CaptureResponse capture(CaptureRequest req) {
        BrowserEnvironment env = req.environment() == null ? BrowserEnvironment.TEST : req.environment();
        ScreenshotStrategy strategy = req.strategy() == null ? ScreenshotStrategy.VIEWPORT : req.strategy();
        ...
        try {
            ...
            CaptureResponse response = locks.doWithLock(h, sess -> {
                try {
                    if (sess.isMobile()) {
                    ...
                    String source = null;
                    if (includeSource) {
                        source = sess.isMobile() ? sess.asMobileDevice().getSource()
    ...
    private ElementStateResponse resolveElement(SessionHandle sess, String xpath) {
        ...
        Map<String, String> attributes = sess.isMobile()
    ...
    private byte[] renderPageScreenshot(SessionHandle sess, ScreenshotStrategy strategy) throws Exception {
        BufferedImage image;
        if (sess.isMobile()) {
    ...
    private static CaptureScreenshotRef buildRef(UUID captureId, byte[] pngBytes, int width, int height, PngEncoding encoding) {
        ...
        return new CaptureScreenshotRef(
                null,
                "/v1/capture/" + captureId + "/screenshot",
                width, height);
    }
